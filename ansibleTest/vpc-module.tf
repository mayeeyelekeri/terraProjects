...
resource "aws_subnet" "private-subnets" {
  ...
  tags = {
    Name = join ("-" , ["${terraform.workspace}", "${each.value.cidr}", "${each.value.zone}"])
    Environment = "${terraform.workspace}"
  }
}
...
# Create NAT Gateway in public1 subnet 
resource "aws_nat_gateway" "nat-gateway" {
  subnet_id     = aws_subnet.public-subnets["subnet1"].id
...
resource "aws_route_table_association" "private-route-table-association" {
  ...
  depends_on = [aws_subnet.private-subnets,  aws_default_route_table.private-route]
}
...
resource "aws_security_group" "private-sg" {
  ...
  ingress {
    ...
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = values(aws_subnet.public-subnets)[*].cidr_block
  }  
  ingress {
