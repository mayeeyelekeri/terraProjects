...
resource "aws_instance" "jenkins-server" {
  ...
  provisioner "local-exec" {
    command = <<EOF
aws ec2 wait instance-status-ok --instance-ids ${self.id} && ansible-playbook --extra-vars 'passed_in_hosts=${self.tags.Name}' ansible_templates/install_jenkins.yaml
EOF
  }
  tags = {
